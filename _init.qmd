...
# iNaturalist
loadINat <- function(projectId, projectName, maxLoops = 1000){
   tryCatch(
     exp = {
      ...
      resultsDf <- data.table::rbindlist(obsList, fill = TRUE)
      resultsDf <- taxas[resultsDf, on = .(id = taxon.id)]
...
# Observation.org
loadObs <- function(locationId, locationName, maxLoops = 1000, date_after = "2025-04-25", date_before = "2025-04-28"){
   tryCatch(
     exp = {
      ...
      resultsDf <- rbindlist(obsList, fill = TRUE)  |>
...
loadAll <- function(){
  ...
  resultsDf <- rbindlist(list(combined_inat_data, combined_obs_data), fill = TRUE, use.names = TRUE)
